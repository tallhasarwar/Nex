...
@IBDesignable class FloatLabelTextView: DesignableTextView {
	...
	var titleFont:UIFont = UIFont(name: "Poppins-Medium", size: 10.0) ?? UIFont.systemFont(ofSize: 10.0) {
		didSet {
	...
	@IBInspectable var titleYPadding:CGFloat = 1.5 {
		didSet {
			var r = title.frame
	...
	@IBInspectable var titleTextColour:UIColor = UIColor(white: 0.2, alpha: 1.0)  {
		didSet {
			if !isFirstResponder {
	...
	@IBInspectable var titleActiveTextColour:UIColor = UIColor(white: 0.2, alpha: 1.0)  {
		didSet {
			if isFirstResponder {
...
//        titleActiveTextColour = tintColor
	...
	fileprivate func adjustTopTextInset() {
		...
		inset.top = title.font.lineHeight + hintYPadding
        inset.left = 15.0
	...
	fileprivate func textRect()->CGRect {
		...
		r.origin.x += textContainer.lineFragmentPadding + 15.0
